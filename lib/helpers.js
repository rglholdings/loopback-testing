...
_beforeEach.withTestDataBuilder = function(TDB) {
  beforeEach(function() {
    this.TDB = TDB;
  });
}
...
_beforeEach.givenUserWithRole = function (attrs, role, optionalHandler) {
  _beforeEach.givenUser(attrs, function (done) {
    ...
    // We can't do a findOrCreate since we need to know if it was created
    // or found so we can clean it up only if created
    Role.findOne({
      where: {
        name: role
      }
      ...
      var existingRole = result;

      if (existingRole) {

        test.userRole = existingRole;
        createRoleMapping();
        ...
        Role.create({
          ...
          test._newRoleCreated = true;
          test.userRole = newRole;
  ...
  afterEach(function(done) {
    ...
    // First delete the RoleMapping
    deleteRoleMapping(function() {

      if (test._newRoleCreated) {

        // Delete the new role that was created
        test.userRole.destroy(function(err) {
          if(err) return done(err);

          test._newRoleCreated = undefined;
          test.userRole = undefined;
          done();
        });
      } else {

        // Just cleanup the data from the existing role
        test._newRoleCreated = undefined;
        test.userRole = undefined;
        done();
      }
  ...
  describe(verb.toUpperCase() + ' ' + urlStr, function() {
    beforeEach(function(cb) {
      ...
      if (data) {
        ...
        } else if (typeof data === 'object' && this.TDB) {

          // Try to resolve any TDB references in the data
          payload = this.TDB._resolveValues(data);
        }
